...
void print_error_context(const parsec::ParseError &error,
  ...
  if (!error_span.is_valid()) {
    if (error_token) {
      std::cerr << "Unexpected token: '" << error_token->value << "'"
                << std::endl;
    } else {
  ...
  std::string token_value =
      error_token ? error_token->value : std::string("<input>");
  std::cerr << "Unexpected token: '" << token_value << "' at "
            << sources.get_filename(error_span.file) << ":" << loc.line << ":"
            << loc.column << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    // The NameResolver now handles skeleton registration internally before name
    // resolution
    ...
    try {
      ...
      // Validate
      if (!opt::mir::validate_pure_node_types(opt_mod, &std::cerr)) {
        std::cerr
      ...
      std::cerr << "Opt MIR Lowering Failed: " << e.what() << "\n";
      return 1;
    }
