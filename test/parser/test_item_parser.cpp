...
T* get_node(const Ptr& ptr) {
    if (!ptr) return nullptr;
    return std::get_if<T>(&(ptr->value));
}

template <typename T, typename Ptr>
T* get_node(const std::optional<Ptr>& opt_ptr) {
    if (!opt_ptr) return nullptr;
    return get_node<T>(*opt_ptr);
}
...
TEST_F(ItemParserTest, ParsesFunctionNoReturnType) {
  ...
  EXPECT_FALSE(fn->return_type.has_value());
...
TEST_F(ItemParserTest, ParsesFunctionWithReturnType) {
    ...
    auto fn = get_node<FunctionItem>(it);
    ASSERT_NE(fn, nullptr);
    ASSERT_TRUE(fn->return_type.has_value());
...
TEST_F(ItemParserTest, ParsesFunctionEmptyBody) {
    ...
    EXPECT_FALSE(fn->return_type.has_value());
    ASSERT_TRUE(fn->body.has_value());
    EXPECT_TRUE(fn->body.value()->statements.empty());
    EXPECT_FALSE(fn->body.value()->final_expr.has_value());
}
...
TEST_F(ItemParserTest, ParsesTraitWithFunction) {
  ...
  ASSERT_EQ(fn->params.size(), 0u);
  ASSERT_TRUE(fn->self_param.has_value());
  EXPECT_FALSE(fn->self_param.value()->is_mutable);
  EXPECT_TRUE(fn->self_param.value()->is_reference);
}
...
TEST_F(ItemParserTest, ParsesInherentImplWithMultipleFunctions) {
  auto it =
      parse_item("impl Point { fn new() -> Point {} fn x(&self) -> i32 {} }");
  auto im = get_node<InherentImplItem>(it);
  ASSERT_NE(im, nullptr);
...
TEST_F(ItemParserTest, FunctionWithSelf) {
...

  {
      ...
      ASSERT_TRUE(func->self_param.has_value());
      auto* self = func->self_param.value().get();
