...
ExprInfo ExprChecker::check(hir::Literal &expr, TypeExpectation exp) {
  return std::visit(
      Overloaded{[&](hir::Literal::Integer &integer) -> ExprInfo {
                   ...
                   if (auto overflow_err =
                           overflow_int_literal_check(integer)) {
                     throw_in_context(std::string(overflow_err->message));
                   }
...
ExprInfo ExprChecker::check(hir::ConstUse &expr, TypeExpectation) {
  ...
  auto &declared_annotation =
      const_cast<hir::TypeAnnotation &>(*expr.def->type);
  TypeId declared_type = context.type_query(declared_annotation);
  ...
  auto &const_def = const_cast<hir::ConstDef &>(*expr.def);
...
ExprInfo ExprChecker::check(hir::FieldAccess &expr, TypeExpectation) {
  ...
  expr.field = *field_id;
  auto *struct_def = const_cast<hir::StructDef *>(struct_type->symbol);
  TypeId type =
...
ExprInfo ExprChecker::check(hir::ArrayRepeat &expr, TypeExpectation exp) {
  ...
  auto resolve_count = [&]() -> size_t {
    ...
    auto const_value =
        context.const_query(**count_expr_ptr,
...
ExprInfo ExprChecker::check(hir::Call &expr, TypeExpectation) {
  ...
  for (size_t i = 0; i < func_type->def->params.size(); ++i) {
    auto &param_annotation = const_cast<hir::TypeAnnotation &>(
        *func_type->def->param_type_annotations[i]);
    TypeId param_type = context.type_query(param_annotation);
    ExprInfo arg_info =
