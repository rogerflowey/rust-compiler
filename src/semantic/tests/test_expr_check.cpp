...
TEST_F(ExprCheckTest, UnaryOpNot) {
    ...
    auto expr = createUnaryOp(std::move(operand), makeUnaryNot());
    auto info = expr_checker->check(*expr);
...
TEST_F(ExprCheckTest, UnaryOpNegate) {
    ...
    auto expr = createUnaryOp(std::move(operand), makeUnaryNegate());
    auto info = expr_checker->check(*expr);
    EXPECT_EQ(info.type, i32_type);
    EXPECT_FALSE(info.is_mut);
    EXPECT_FALSE(info.is_place);
...
TEST_F(ExprCheckTest, UnaryOpDereference) {
    auto operand = createUnaryOp(createVariable(test_local_i32.get()), makeReference(true));
    auto expr = createUnaryOp(std::move(operand), makeDereference());
    auto info = expr_checker->check(*expr);
    EXPECT_EQ(info.type, i32_type);
...
TEST_F(ExprCheckTest, BinaryOpAdd) {
    ...
    auto expr = createBinaryOp(std::move(lhs), std::move(rhs), makeAdd());
...
TEST_F(ExprCheckTest, BinaryOpEqual) {
    ...
    auto expr = createBinaryOp(std::move(lhs), std::move(rhs), makeEqual());
...
TEST_F(ExprCheckTest, BinaryOpLogicalAnd) {
    ...
    auto expr = createBinaryOp(std::move(lhs), std::move(rhs), makeLogicalAnd());
...
TEST_F(ExprCheckTest, BinaryOpBitwiseAnd) {
    auto lhs = createIntegerLiteral(42, ast::IntegerLiteralExpr::I32);
    auto rhs = createIntegerLiteral(24, ast::IntegerLiteralExpr::I32);
    auto expr = createBinaryOp(std::move(lhs), std::move(rhs), makeBitAnd());
...
TEST_F(ExprCheckTest, BinaryOpShiftLeft) {
    ...
    auto expr = createBinaryOp(std::move(lhs), std::move(rhs), makeShiftLeft());
