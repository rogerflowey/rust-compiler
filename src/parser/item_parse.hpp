...
public:
  ...
  void finalize(const ParserRegistry &registry,
    ...
    const auto &selfParser =
        registry.item; // For recursion in trait/impl bodies
...
private:
  ...
  buildBlockParser(const StmtParser &stmtParser,
    ...
    return (equal({TOKEN_DELIMITER, "{"}) > stmtParser.many().andThen(tail) <
        ...
        .map([](auto &&tp) -> BlockExprPtr {
          ...
          return std::make_unique<BlockExpr>(std::move(stmts), std::move(opt));
        });
  }
  ...
  ItemParser buildFunctionParser(
    ...
    // --- Parameter Component Parsers ---

    // SelfParam parser for the shorthand form: `&? mut? self`
    auto selfParam = p_amp.andThen(p_mut)
                         .andThen(p_self_tok)
                         .map([](auto &&t) -> SelfParamPtr {
                           auto &[amp, mut, _] = t;
                           return std::make_unique<SelfParam>(
                               amp.has_value(), mut.has_value());
                         });

    auto funcParam =
        (p_identifier.andThen(equal({TOKEN_SEPARATOR, ":"}) > typeParser))
            .map([](auto &&p) {
              return std::pair<IdPtr, TypePtr>(std::move(std::get<0>(p)),
                                               std::move(std::get<1>(p)));
            });

    auto p_only_self =
        selfParam
            .keepLeft(p_comma.optional()) // Consume an optional trailing comma
            .map([](auto &&self) {
              // Map the result to the common pair format for consistency
              return std::make_pair(std::move(self),
                                    std::vector<std::pair<IdPtr, TypePtr>>{});
            });

    auto p_func_param_list = funcParam.tuple(p_comma);
    auto p_self_prefix =
        (selfParam < p_comma)
            .optional(); // Note: self MUST be followed by a comma here

    auto p_with_regular_params =
        p_self_prefix.andThen(p_func_param_list)
            .keepLeft(p_comma.optional()) // Consume an optional trailing comma
            .map([](auto &&t) {
    ...
    auto params =
        (equal({TOKEN_DELIMITER, "("}) > params_content.optional() <
         equal({TOKEN_DELIMITER, ")"}))
            .map([](auto &&mv) {
              if (mv) {
                return std::move(*mv);
              }
              return std::make_pair(SelfParamPtr(nullptr),
                                    std::vector<std::pair<IdPtr, TypePtr>>{});
            });
  ...
  ItemParser buildStructParser(const TypeParser &typeParser) const {
    ...
    auto fields_list =
        field.tuple(equal({TOKEN_SEPARATOR, ","}))
            .optional()
            .map([](auto &&mv) {
              return mv ? std::move(*mv)
                        : std::vector<std::pair<IdPtr, TypePtr>>{};
            });

    auto brace_fields = (equal({TOKEN_DELIMITER, "{"}) > fields_list <
                         equal({TOKEN_DELIMITER, "}"}));
    auto unit_struct = equal({TOKEN_SEPARATOR, ";"}).map([](auto) {
      return std::vector<std::pair<IdPtr, TypePtr>>{};
    });

    auto fields = brace_fields | unit_struct;

    return (equal({TOKEN_KEYWORD, "struct"}) > p_identifier)
        .andThen(fields)
        .map([](auto &&t) -> ItemPtr {
          return std::make_unique<StructItem>(std::move(std::get<0>(t)),
  ...
  ItemParser buildEnumParser() const {
    auto variants =
        (equal({TOKEN_DELIMITER, "{"}) >
         p_identifier.tuple(equal({TOKEN_SEPARATOR, ","})).optional() <
         equal({TOKEN_DELIMITER, "}"}))
            .map([](auto &&mv) {
              return mv ? std::move(*mv) : std::vector<IdPtr>{};
            });
    return (equal({TOKEN_KEYWORD, "enum"}) > p_identifier)
        .andThen(variants)
        .map([](auto &&t) -> ItemPtr {
          return std::make_unique<EnumItem>(std::move(std::get<0>(t)),
                                            std::move(std::get<1>(t)));
        })
        .label("an enum definition");
  }

  ItemParser buildConstParser(const TypeParser &typeParser,
                              const ExprParser &exprParser) const {
    return (equal({TOKEN_KEYWORD, "const"}) > p_identifier)
        ...
        .map([](auto &&t) -> ItemPtr {
          auto &[name, ty, val] = t;
          return std::make_unique<ConstItem>(std::move(name), std::move(ty),
                                             std::move(val));
        })
        .label("a const item");
  }
  ...
  ItemParser buildTraitParser(const ItemParser &self) const {
    auto bodyItems = (equal({TOKEN_DELIMITER, "{"}) > self.many() <
                      equal({TOKEN_DELIMITER, "}"}));
    return (equal({TOKEN_KEYWORD, "trait"}) > p_identifier)
        .andThen(bodyItems)
        .map([](auto &&t) -> ItemPtr {
          auto name = std::move(std::get<0>(t));
          auto items = std::move(std::get<1>(t));
          return std::make_unique<TraitItem>(std::move(name), std::move(items));
        })
        .label("a trait definition");
  }

  ItemParser buildImplParser(const TypeParser &typeParser,
                             const ItemParser &self) const {
    auto optTrait = (p_identifier < equal({TOKEN_KEYWORD, "for"})).optional();
    auto bodyItems = (equal({TOKEN_DELIMITER, "{"}) > self.many() <
                      equal({TOKEN_DELIMITER, "}"}));
    return (equal({TOKEN_KEYWORD, "impl"}) > optTrait)
        .andThen(typeParser)
        .andThen(bodyItems)
        .map([](auto &&t) -> ItemPtr {
          auto &[optName, forType, items] = t;
          if (optName) {
            return std::make_unique<TraitImplItem>(
                std::move(*optName), std::move(forType), std::move(items));
          } else {
            return std::make_unique<InherentImplItem>(std::move(forType),
                                                      std::move(items));
          }
        })
        .label("an impl block");
