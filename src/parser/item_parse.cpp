...
void ItemParserBuilder::finalize(
  ...
  auto functionParser = buildFunctionParser(patternParser, typeParser, blockParser);
...
ItemParser ItemParserBuilder::buildFunctionParser(
    const PatternParser &patternParser, const TypeParser &typeParser,
  ...
  auto funcParam =
      (patternParser.andThen(equal({TOKEN_SEPARATOR, ":"}) > typeParser))
          .map([](auto &&p) {
            return std::pair<PatternPtr, TypePtr>(std::move(std::get<0>(p)),
...
ItemParser
ItemParserBuilder::buildStructParser(const TypeParser &typeParser) const {
  auto field =
  ...
  auto fields_list =
      field.tuple(equal({TOKEN_SEPARATOR, ","})).optional().map([](auto &&mv) {
  ...
  return (equal({TOKEN_KEYWORD, "struct"}) > p_identifier)
      ...
      .map([](auto &&t) -> ItemPtr {
        return std::make_unique<Item>(Item{
            StructItem{std::move(std::get<0>(t)), std::move(std::get<1>(t))}});
      })
...
ItemParser ItemParserBuilder::buildEnumParser() const {
  ...
  return (equal({TOKEN_KEYWORD, "enum"}) > p_identifier)
      ...
      .map([](auto &&t) -> ItemPtr {
        return std::make_unique<Item>(Item{
            EnumItem{std::move(std::get<0>(t)), std::move(std::get<1>(t))}});
      })
      .label("an enum definition");
}

ItemParser
ItemParserBuilder::buildConstParser(const TypeParser &typeParser,
                                    const ExprParser &exprParser) const {
  return (equal({TOKEN_KEYWORD, "const"}) > p_identifier)
      ...
      .map([](auto &&t) -> ItemPtr {
        ...
        return std::make_unique<Item>(
            Item{ConstItem{std::move(name), std::move(ty), std::move(val)}});
      })
      .label("a const item");
}
...
ItemParser ItemParserBuilder::buildTraitParser(const ItemParser &self) const {
  ...
  return (equal({TOKEN_KEYWORD, "trait"}) > p_identifier)
      ...
      .map([](auto &&t) -> ItemPtr {
        return std::make_unique<Item>(Item{
            TraitItem{std::move(std::get<0>(t)), std::move(std::get<1>(t))}});
      })
      .label("a trait definition");
}

ItemParser ItemParserBuilder::buildImplParser(const TypeParser &typeParser,
  ...
  return (equal({TOKEN_KEYWORD, "impl"}) > optTrait)
      ...
      .map([](auto &&t) -> ItemPtr {
        ...
        if (optName) {
          return std::make_unique<Item>(Item{TraitImplItem{
              std::move(*optName), std::move(forType), std::move(items)}});
        } else {
          return std::make_unique<Item>(
              Item{InherentImplItem{std::move(forType), std::move(items)}});
        }
      })
      .label("an impl block");
}