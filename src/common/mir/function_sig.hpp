...
struct AbiParamByValCallerCopy {}; // Caller allocates and manages byval copy,
                                   // callee receives pointer (no-escape)
struct AbiParamSRet {};

// ABI parameter representation
struct AbiParam {
  std::optional<ParamIndex>
      param_index; // which semantic param this implements (if any)
...
inline TypeId return_type(const ReturnDesc &r) {
  return std::visit(
      Overloaded{[](const ReturnDesc::RetDirect &k) { return k.type; },
