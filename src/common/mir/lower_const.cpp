...
namespace mir {
namespace detail {
StringConstant make_string_constant(const std::string &literal,
                                    bool is_cstyle_literal) {
...
ConstantValue convert_literal_value(const hir::Literal::Integer &integer) {
  ...
  constant.is_signed =
      integer.suffix_type != ast::IntegerLiteralExpr::NOT_SPECIFIED;
  return constant;
}
...
ConstantValue convert_const_value(const semantic::IntConst &value) {
  ...
  constant.value = constant.is_negative ? static_cast<uint64_t>(-signed_value)
...
Constant lower_const_definition(const hir::ConstDef &const_def, TypeId type) {
  if (type == invalid_type_id) {
    throw std::logic_error(
        "Const definition missing resolved type during MIR lowering");
  }
  semantic::ConstVariant value = hir::helper::get_const_value(const_def);
  Constant constant;
  constant.type = canonicalize_type_for_mir(type);
  constant.value = std::visit(
      [&](const auto &variant) -> ConstantValue {
...
Constant lower_enum_variant(const hir::EnumVariant &enum_variant, TypeId type) {
  ...
  if (type == invalid_type_id) {
    throw std::logic_error(
        "Enum variant missing resolved type during MIR lowering");
  }
