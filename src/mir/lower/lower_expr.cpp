...
mir::Operand LowerResult::as_operand(FunctionLowerer& fl, mir::TypeId type) {
  switch (kind) {
    case Kind::Operand:
      if (!std::holds_alternative<mir::Operand>(data)) {
        throw std::logic_error("Inconsistent LowerResult state");
      }
      return std::move(std::get<mir::Operand>(data));
    
    case Kind::Place: {
      if (!std::holds_alternative<mir::Place>(data)) {
        throw std::logic_error("Inconsistent LowerResult state");
      }
      mir::Place place = std::move(std::get<mir::Place>(data));
      return fl.load_place_value(std::move(place), type);
    }
    
    case Kind::Written:
      throw std::logic_error("Cannot materialize as operand: value was already written to destination");
  }
...
mir::Place LowerResult::as_place(FunctionLowerer& fl, mir::TypeId type) {
  switch (kind) {
    case Kind::Place:
      ...
      return std::move(std::get<mir::Place>(data));
    
    case Kind::Operand: {
      if (!std::holds_alternative<mir::Operand>(data)) {
        throw std::logic_error("Inconsistent LowerResult state");
      }
      mir::Operand op = std::move(std::get<mir::Operand>(data));
      mir::TempId temp = fl.allocate_temp(type);
      mir::AssignStatement assign;
      assign.dest = mir::Place{mir::LocalPlace{temp}};
      assign.src = mir::ValueSource{std::move(op)};
      mir::Statement stmt;
      stmt.value = std::move(assign);
      fl.append_statement(std::move(stmt));
      return mir::Place{mir::LocalPlace{temp}};
    }
    
    case Kind::Written:
      throw std::logic_error("Cannot materialize as place: value was already written to destination");
  }
  throw std::logic_error("Unreachable");
}
...
void LowerResult::write_to_dest(FunctionLowerer& fl, mir::Place dest, mir::TypeId type) {
  switch (kind) {
    case Kind::Written:
      // No-op: hint was taken by lowering
      return;
    ...
    case Kind::Place: {
      ...
      mir::Place src_place = std::move(std::get<mir::Place>(data));
      // For place-to-place copy, load the source and assign to dest
      mir::Operand loaded_op = fl.load_place_value(std::move(src_place), type);
      mir::AssignStatement assign;
      assign.dest = std::move(dest);
      assign.src = mir::ValueSource{std::move(loaded_op)};
      mir::Statement stmt;
      stmt.value = std::move(assign);
      fl.append_statement(std::move(stmt));
      return;
...
LowerResult FunctionLowerer::lower_expr_impl(const hir::StructLiteral& struct_literal,
                                             const semantic::ExprInfo& info,
                                             std::optional<Place> dest) {
  // StructLiteral is dest-aware: try to write directly to dest if provided
  if (!dest) {
    // No dest hint: compute normally and return as operand
    auto result = lower_expr_impl(struct_literal, info);
    return LowerResult::operand(std::move(*result));
  }
  
  // We have a dest hint - initialize it directly
  TypeId normalized = canonicalize_type_for_mir(info.type);
  lower_struct_init(struct_literal, *dest, normalized);
...
LowerResult FunctionLowerer::lower_expr_impl(const hir::ArrayLiteral& array_literal,
                                             const semantic::ExprInfo& info,
                                             std::optional<Place> dest) {
  // ArrayLiteral is dest-aware: try to write directly to dest if provided
  if (!dest) {
    // No dest hint: compute normally and return as operand
    auto result = lower_expr_impl(array_literal, info);
    return LowerResult::operand(std::move(*result));
  }
  
  // We have a dest hint - initialize it directly
  TypeId normalized = canonicalize_type_for_mir(info.type);
  lower_array_literal_init(array_literal, *dest, normalized);
  return LowerResult::written();
}
...
LowerResult FunctionLowerer::lower_expr_impl(const hir::ArrayRepeat& array_repeat,
                                             const semantic::ExprInfo& info,
                                             std::optional<Place> dest) {
  // ArrayRepeat is dest-aware: try to write directly to dest if provided
  if (!dest) {
    // No dest hint: compute normally and return as operand
    auto result = lower_expr_impl(array_repeat, info);
    return LowerResult::operand(std::move(*result));
  }
  
  // We have a dest hint - initialize it directly
  TypeId normalized = canonicalize_type_for_mir(info.type);
  lower_array_repeat_init(array_repeat, *dest, normalized);
  return LowerResult::written();
}

// Stub implementations for all other new-style overloads
// These wrap the old-style implementations for now

LowerResult FunctionLowerer::lower_expr_impl(const hir::Variable& variable,
                                             const semantic::ExprInfo& info,
                                             std::optional<Place>) {
  auto result = lower_expr_impl(variable, info);
  return LowerResult::operand(std::move(*result));
}
...
LowerResult FunctionLowerer::lower_expr_impl(const hir::ConstUse& const_use,
                                             const semantic::ExprInfo& info,
                                             std::optional<Place>) {
  auto result = lower_expr_impl(const_use, info);
  return LowerResult::operand(std::move(*result));
}
...
LowerResult FunctionLowerer::lower_expr_impl(const hir::StructConst& struct_const,
                                             const semantic::ExprInfo& info,
                                             std::optional<Place>) {
  auto result = lower_expr_impl(struct_const, info);
  return LowerResult::operand(std::move(*result));
}
...
LowerResult FunctionLowerer::lower_expr_impl(const hir::EnumVariant& enum_variant,
                                             const semantic::ExprInfo& info,
                                             std::optional<Place>) {
  auto result = lower_expr_impl(enum_variant, info);
  return LowerResult::operand(std::move(*result));
}
...
LowerResult FunctionLowerer::lower_expr_impl(const hir::FieldAccess& field_access,
                                             const semantic::ExprInfo& info,
                                             std::optional<Place>) {
  auto result = lower_expr_impl(field_access, info);
  return LowerResult::operand(std::move(*result));
}
...
LowerResult FunctionLowerer::lower_expr_impl(const hir::Index& index_expr,
                                             const semantic::ExprInfo& info,
                                             std::optional<Place>) {
  auto result = lower_expr_impl(index_expr, info);
  return LowerResult::operand(std::move(*result));
}
...
LowerResult FunctionLowerer::lower_expr_impl(const hir::Cast& cast_expr,
                                             const semantic::ExprInfo& info,
                                             std::optional<Place>) {
  auto result = lower_expr_impl(cast_expr, info);
  return LowerResult::operand(std::move(*result));
}
...
LowerResult FunctionLowerer::lower_expr_impl(const hir::BinaryOp& binary,
                                             const semantic::ExprInfo& info,
                                             std::optional<Place>) {
  auto result = lower_expr_impl(binary, info);
  return LowerResult::operand(std::move(*result));
}
...
LowerResult FunctionLowerer::lower_expr_impl(const hir::Assignment& assignment,
                                             const semantic::ExprInfo& info,
                                             std::optional<Place>) {
  auto result = lower_expr_impl(assignment, info);
  return LowerResult::operand(std::move(*result));
}
...
LowerResult FunctionLowerer::lower_expr_impl(const hir::Block& block_expr,
                                             const semantic::ExprInfo& info,
                                             std::optional<Place> dest) {
  // Blocks propagate dest hint
  if (!dest) {
    auto result = lower_expr_impl(block_expr, info);
    return result ? LowerResult::operand(std::move(*result)) : LowerResult::written();
  }
  // TODO: Propagate dest through block statements
  auto result = lower_expr_impl(block_expr, info);
  return result ? LowerResult::operand(std::move(*result)) : LowerResult::written();
}
...
LowerResult FunctionLowerer::lower_expr_impl(const hir::If& if_expr,
                                             const semantic::ExprInfo& info,
                                             std::optional<Place> dest) {
  // If expressions should propagate dest hint
  if (!dest) {
    auto result = lower_expr_impl(if_expr, info);
    return result ? LowerResult::operand(std::move(*result)) : LowerResult::written();
  }
  // TODO: Propagate dest through if branches
  auto result = lower_expr_impl(if_expr, info);
  return result ? LowerResult::operand(std::move(*result)) : LowerResult::written();
}
...
LowerResult FunctionLowerer::lower_expr_impl(const hir::Loop& loop_expr,
                                             const semantic::ExprInfo& info,
                                             std::optional<Place> dest) {
  (void)dest;  // Loops don't use dest hint
  auto result = lower_expr_impl(loop_expr, info);
  return result ? LowerResult::operand(std::move(*result)) : LowerResult::written();
}
...
LowerResult FunctionLowerer::lower_expr_impl(const hir::While& while_expr,
                                             const semantic::ExprInfo& info,
                                             std::optional<Place> dest) {
  (void)dest;  // While loops don't use dest hint
  auto result = lower_expr_impl(while_expr, info);
  return result ? LowerResult::operand(std::move(*result)) : LowerResult::written();
}

LowerResult FunctionLowerer::lower_expr_impl(const hir::Break& break_expr,
                                             const semantic::ExprInfo& info,
                                             std::optional<Place> dest) {
  (void)dest;  // Breaks don't use dest hint
  auto result = lower_expr_impl(break_expr, info);
  return result ? LowerResult::operand(std::move(*result)) : LowerResult::written();
}

LowerResult FunctionLowerer::lower_expr_impl(const hir::Continue& continue_expr,
                                             const semantic::ExprInfo& info,
                                             std::optional<Place> dest) {
  (void)dest;  // Continues don't use dest hint
  auto result = lower_expr_impl(continue_expr, info);
  return result ? LowerResult::operand(std::move(*result)) : LowerResult::written();
}
...
LowerResult FunctionLowerer::lower_expr_impl(const hir::Return& return_expr,
                                             const semantic::ExprInfo& info,
                                             std::optional<Place> dest) {
  (void)dest;  // Returns don't use dest hint
  auto result = lower_expr_impl(return_expr, info);
  return result ? LowerResult::operand(std::move(*result)) : LowerResult::written();
}

// Additional stubs for Call, MethodCall, UnaryOp
LowerResult FunctionLowerer::lower_expr_impl(const hir::Call& call_expr,
                                             const semantic::ExprInfo& info,
                                             std::optional<Place> dest) {
  (void)dest;  // Calls don't use dest hint yet (TODO: support SRET)
  auto result = lower_expr_impl(call_expr, info);
  return result ? LowerResult::operand(std::move(*result)) : LowerResult::written();
}

LowerResult FunctionLowerer::lower_expr_impl(const hir::MethodCall& method_call,
                                             const semantic::ExprInfo& info,
                                             std::optional<Place> dest) {
  (void)dest;  // Method calls don't use dest hint yet (TODO: support SRET)
...
LowerResult FunctionLowerer::lower_expr_impl(const hir::UnaryOp& unary,
                                             const semantic::ExprInfo& info,
                                             std::optional<Place>) {
  auto result = lower_expr_impl(unary, info);
  return LowerResult::operand(std::move(*result));
}
...
// Unified expression lowering with destination hint
// This is now the main entry point accepting optional destination
LowerResult FunctionLowerer::lower_expr_with_dest(const hir::Expr &expr, std::optional<Place> maybe_dest) {
  semantic::ExprInfo info = hir::helper::get_expr_info(expr);

  bool was_reachable = is_reachable();

  // Dispatch to dest-aware lower_expr_impl implementations
  auto result = std::visit(
...
std::optional<Operand> FunctionLowerer::lower_expr(const hir::Expr &expr) {
  // Legacy compatibility: lower_expr without dest hint
  semantic::ExprInfo info = hir::helper::get_expr_info(expr);

  bool was_reachable = is_reachable();

  // Dispatch to old lower_expr_impl (returns optional<Operand>)
  auto result = std::visit(
      [this, &info](const auto &node) { return lower_expr_impl(node, info); },
      expr.value);
